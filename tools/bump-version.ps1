...
if ($Check) {
    ...
    if (-not $manifestAddOnVersion) {
        Write-Error "Missing ## AddOnVersion in EZOAuto.txt"
        $ok = $false
    }
        ...
        try {
            ...
            foreach ($expectedApiToken in $expectedApiTokens) {
                if ($manifestApiTokens -notcontains $expectedApiToken) {
                    Write-Error "APIVersion mismatch: EZOAuto.txt='$manifestApiVersion' does not include expected API '$expectedApiToken'"
                    $ok = $false
                }
            }
        } catch {
